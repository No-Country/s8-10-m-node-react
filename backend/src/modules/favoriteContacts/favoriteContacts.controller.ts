...
export class FavoriteContactController extends FavoriteContactServices {
  ...
  async getAllController(req: Request, res: Response) {
    ...
    try {
      if (!user) return httpError.response(res, 400, "User not found");
  ...
  async postController(req: Request, res: Response) {
    ...
    try {
      ...
      if (!accountUser) return httpError.response(res, 400, "Account not found");
  ...
  async deleteController(req: Request, res: Response) {
    ...
    try {
      ...
      if (!contact) return httpError.response(res, 400, "Favorite contact not found");
